...
object Maps:
  ...
  object Path:
    ...
    private def buildPath[E](offset: Int, script: MapScript[E]): Path[E] = script match {
      case levels: MapScript[E] if (!levels.isEmpty) =>
        levels.head match {
          case level: MapLevel[E] if (level.nonEmpty) =>
            findBranch(offset, level) match {
              case MapBranch.Node(o, event) => Node(event, buildPath(o, MapScript(levels.tail)))
              case MapBranch.Fork(o, left, right) =>
                o match {
                  case 3 =>
                    Fork(
