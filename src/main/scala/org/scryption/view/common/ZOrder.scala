...
/** Shared back-to-front stacking rule for hand-of-cards views: cards sit in their natural index order, the
  * hovered-but-not-slotted card floats above those, and whichever card is in the slot is always frontmost of all. Every
  * view with a hand and a slot (Strange Stones, both fire camp views, Mycologists) had this exact algorithm
  * copy-pasted; it's now defined once here.
  *
  * `labelsOf` returns every Swing label a single item owns (usually one, but Mycologists' duplicate-card slot owns
  * two), so both cases can share this.
  */
object ZOrder {
